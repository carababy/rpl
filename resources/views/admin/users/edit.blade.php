...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="gender_id">{{ trans('cruds.user.fields.gender') }}</label>
                <select class="form-control select2 {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender_id" id="gender_id" required>
                    @foreach($genders as $id => $gender)
                        <option value="{{ $id }}" {{ (old('gender_id') ? old('gender_id') : $user->gender->id ?? '') == $id ? 'selected' : '' }}>{{ $gender }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="cooperative">{{ trans('cruds.user.fields.cooperative') }}</label>
                <input class="form-control {{ $errors->has('cooperative') ? 'is-invalid' : '' }}" type="text" name="cooperative" id="cooperative" value="{{ old('cooperative', $user->cooperative) }}">
                @if($errors->has('cooperative'))
                    <span class="text-danger">{{ $errors->first('cooperative') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.cooperative_helper') }}</span>
            </div>
            <div class="form-group">
